...
class SuggestedSearch extends Component {
  ...
  handleClear = (ev) => {
    ...
    // this.props.setCurrentLocationWithLevel(null);
  }
    ...
    return (<div style={this.props.style}>
      <FormControl className={classes.search} onFocus={this.handleFocus} onBlur={this.handleBlur} fullWidth>
        <Input inputProps={{
          ...
          }} placeholder='Поиск по названию или ключевым словам' style={{
            borderBottom: '1px solid #ccc'
          }} value={this.state.inputValue}
          // startAdornment={<InputAdornment position = "start" ><Search/></InputAdornment>}
          endAdornment={(
            this.state.inputValue !== '')
            ? <InputAdornment position="end" onClick={this.handleClear}>
              <IconButton><Clear/></IconButton>
            </InputAdornment>
            : null} onChange={this.handleChange}/>
      </FormControl>
      ...
      <Paper className={classes.paper} square>
        {
          getSuggestions(this.state.inputValue, this.state.locations).map((suggestion, index) => {
            ...
            if (suggestion.matches[0].key !== 'title') {
              hint = (
                (suggestion.matches[0].key === 'category.name')
                ? 'Категория: '
                : 'Ключевое слово: ') + suggestion.matches[0].value
            } else
              hint = null;
            return (
              (this.state.inputValue !== suggestion.item.title)
              ? <ListItem button key={index} component="div" id={suggestion.item.name} onClick={() => {
                  this.handleSelect({title: suggestion.item.title, id: suggestion.item.name,level:suggestion.item.level})
                }}>
      ...
      </Paper>
      {
        (this.state.showKeyboard)
          ? <Keyboard onClick={(value) => {
              ...
              }} defaultKeyboard={this.props.defaultKeyboard} secondaryKeyboard={this.props.secondaryKeyboard} inputNode={this.state.input} dataset={this.props.dataset} isFirstLetterUppercase={this.props.isFirstLetterUppercase}/>
          : null
      }
    </div>
    );
